		...
		if ($Validator->validate()){
			if ( $res=merchantApp::validateToken($this->data['mtid'],
				...
				if ( $res=$DbExt->rst($stmt)){					
					...
					/*$unopen_count = merchantApp::getUnOpenOrder();
					$unopen_msg = '';
					if($unopen_count){
						$unopen_msg = Yii::t("merchantapp-backend","[total] New Order has been placed.",array(
						  '[total]'=>$unopen_count['total_unopen']
						));
					}*/
					...
					$this->details=array(
					  ...
					  /*'unopen'=>isset($unopen_count['total_unopen'])?$unopen_count['total_unopen']:0,
					  'unopen_msg'=>$unopen_msg*/
					);
