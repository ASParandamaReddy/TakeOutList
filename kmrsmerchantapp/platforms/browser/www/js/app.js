		...
		if (data.code==1){
			...
			switch (action)
			{
				...
				case "GetTodaysOrder":
				   ...
				   //alert('set total_order'+  data.details.total_order);
				   
				   /*UNOPEN ORDER*/
				   //runAlert();
				   
				   /*UNOPEN ORDER*/
				   /*if ( data.details.unopen >=1){
				      if(isDebug()){
			     		 dump( data.details.unopen_msg );
			     	  } else {
			     		 toastMsg(data.unopen_msg);
			     		 playNotification();
			     	  }	     	
				   }*/							   	   
				break;
...
function getUnOPenOrder()
{
   ...
   ajax_request_alert = $.ajax({
	...
	success: function (data) {	  
	     ...
	     var order_list = $(".new-orders").html();
	     ...
	     if ( data.code==1){	     	
	     	...
	     	if ($('.new-orders').is(':visible')) {	     
	     		...
	     		/*alert("data.details.total_order=>"+data.details.total_order);
	     		alert("total_order=>"+total_order);*/
	     		if ( data.details.total_order>total_order){	     			
	     			//alert("REFRESH");
	     			OrderRefresh();
	     		}	     		
	        }	     	
...
function runAlert()
{
	...
	if(empty(alert_handle)){
	   alert_handle = setInterval(function(){getUnOPenOrder()}, alert_delayed );
	}
}
